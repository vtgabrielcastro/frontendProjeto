...
const Crud = () => {
    ...
    const deleteUsuario = () => {
        usuarioService.excluir(usuario.id).then((response) => {
            ...
            toast.current.show({
                severity: 'success',
                summary: 'Sucesso!',
                detail: 'Usuário:' + usuario.nome + ' - deletado com sucesso!',
                life: 3000
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const deleteSelectedUsuarios = () => {
        // let _products = (usuarios as any)?.filter((val: any) => !(selectedProducts as any)?.includes(val));
